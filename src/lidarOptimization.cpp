...
EdgeAnalyticCostFunction::EdgeAnalyticCostFunction(Eigen::Vector3d curr_point_, Eigen::Vector3d last_point_a_, Eigen::Vector3d last_point_b_, double point_weight_)
    : curr_point(curr_point_), last_point_a(last_point_a_), last_point_b(last_point_b_), point_weight(point_weight_)
{
}
...
bool EdgeAnalyticCostFunction::Evaluate(double const *const *parameters, double *residuals, double **jacobians) const
{
    ...
    Eigen::Vector3d lp; // 当前匹配点的上一帧对应点
    lp = q_last_curr * curr_point + t_last_curr;

    Eigen::Vector3d nu = (lp - last_point_a).cross(lp - last_point_b);
    Eigen::Vector3d de = last_point_a - last_point_b;
    double de_norm = de.norm();
    auto residual = nu.norm() / de_norm;
    residuals[0] = residual;
    if (point_weight == 1 || point_weight == 2)
        residuals[0] = point_weight * residuals[0];
    else if (point_weight == 12)
        residuals[0] = point_weight * residuals[0];
...
SurfNormAnalyticCostFunction::SurfNormAnalyticCostFunction(Eigen::Vector3d curr_point_, Eigen::Vector3d plane_unit_norm_, double negative_OA_dot_norm_, double point_weight_)
    : curr_point(curr_point_), plane_unit_norm(plane_unit_norm_), negative_OA_dot_norm(negative_OA_dot_norm_), point_weight(point_weight_)
{
}
